...
class Rep_ventas_por_punto extends CI_Controller {
	...
	public function index($opc_id){
		...
		if($_POST){
			...
			$cns_facturas=$this->rep_ventas_por_punto_model->lista_factura_buscador_2($f1,$f2,$empresa,$emisor);
		}else{
			$empresa='1';
			$emisor = '6';
			$vendedor= 'on';
			...
			foreach ($cns_facturas as $factura) {
				...
				if($gr!=$factura->emi_id){
					$detalle.="<tr class='local'>
								...
								<td colspan='25'>  <b>$factura->emi_nombre</b> </td>
							</tr>";
				}	
				if($vendedor=='checked'){
					if($gr_2!=$factura->cja_id){
						$detalle.="<tr class='caja'>
									<td colspan='25'>$factura->cja_nombre</td>
								</tr>";
					}
				}
				

				$nota=$this->rep_ventas_por_punto_model->lista_nota_credito_2($f1,$f2,$factura->emp_id,$factura->emi_id,$factura->cja_id,$factura->vnd_id);
				if(empty($nota->nc)){
					$nota= (object) array(
								'nc'=>0,
								'subtotal12'=>0,
				...
				$caja=$this->rep_ventas_por_punto_model->lista_cierre_caja_2($f1,$f2,$factura->emp_id,$factura->emi_id,$factura->cja_id,$factura->vnd_id);
		...
		$detalle.="<tr class='total'>
			...
			$cns_emisores=$this->emisor_model->lista_emisores_estado_2('1');	
			$data=array(
